...
$pageTitle = 'Best Seller - FoodShop';
include INCLUDES_PATH . '/header.php';

$conn = getDBConnection();
$sql = "SELECT * FROM bestseller ORDER BY created_at DESC";
...
<section class="category-bar">
    <div class="category-inner">
        ...
        <a class="category-btn active" href="<?php echo CATEGORY_URL; ?>/bestseller.php">Best Seller</a>
    ...
    <div class="product-grid">
        ...
        if ($result->num_rows > 0) {
            while($row = $result->fetch_assoc()) {
                ...
                echo '        <span class="badge hot">Best</span>';
                echo '    </div>';
                ...
                echo '                <button class="btn-add" data-id="'. $row['product_id'] .'" data-table="bestseller">+</button>';
                echo '            </div>';
                echo '        </div>';
                echo '    </div>';
                echo '</article>';
