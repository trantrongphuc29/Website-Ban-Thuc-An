...
$pageTitle = 'Khuyến mãi - FoodShop';
include INCLUDES_PATH . '/header.php';

// Kết nối database
$conn = getDBConnection();

// Lấy sản phẩm từ bảng khuyenmai
$sql = "SELECT * FROM khuyenmai ORDER BY created_at DESC";
...
<section class="hero">
    <div class="hero-content">
        <h1>Khuyến mãi</h1>
        <p>Các chương trình ưu đãi và giảm giá hấp dẫn.</p>
    </div>
</section>
...
<section class="category-bar">
    <div class="category-inner">
        <a class="category-btn active" href="<?php echo CATEGORY_URL; ?>/khuyenmai.php">Khuyến mãi</a>
    ...
    <div class="product-grid">
        ...
        if ($result->num_rows > 0) {
            while($row = $result->fetch_assoc()) {
                ...
                echo '                <button class="btn-add" data-id="'. $row['product_id'] .'" data-table="khuyenmai">+</button>';
                echo '            </div>';
                echo '        </div>';
                echo '    </div>';
                echo '</article>';
